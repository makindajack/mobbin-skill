---
mode: agent
description: Research UI patterns with the Mobbin MCP server. Translates abstract UX goals into concrete screen searches and synthesizes findings into a linkable, decision-ready summary.
---

<!--
This file is the VS Code Copilot Chat / Copilot CLI install of the Mobbin skill.
The body below is kept in sync with the root SKILL.md — only the frontmatter above
differs (VS Code requires `mode` + `description`; the root file uses Anthropic skill
frontmatter with `name`).

To install:
  ln -sf "$PWD/clients/copilot/mobbin.prompt.md" \
    "$HOME/Library/Application Support/Code/User/prompts/mobbin.prompt.md"

Then invoke from Copilot Chat with `/mobbin`.
-->
